...
export async function GET(req: NextRequest) {
  return safeHandler("crm/GET", async () => {
    ...
    const { totalCustomers, newThisMonth, topCustomers, avgStats, vip, silver, bronze, newCustomers, monthly, feedbackStats } =
      await withTenant(session.tenantId, session.userId, async (tx) => {
        ...
        const [vip, silver, bronze, newCustomers] = await Promise.all([
          tx.customer.count({ where: { tenantId: tid, loyaltyPoints: { gte: 5000 } } }),
          tx.customer.count({ where: { tenantId: tid, loyaltyPoints: { gte: 1000, lt: 5000 } } }),
          tx.customer.count({ where: { tenantId: tid, loyaltyPoints: { lt: 1000, gt: 0 } } }),
          tx.customer.count({ where: { tenantId: tid, totalVisits: { lte: 1 } } }),
        ]);

        const monthlyRaw = await tx.$queryRaw<{ month: string; count: bigint }[]>`
        ...
        const feedbackStats = await tx.customerFeedback.aggregate({
          where: { tenantId: tid },
          _avg: { rating: true },
          _count: { id: true },
        });

        return { totalCustomers, newThisMonth, topCustomers, avgStats, vip, silver, bronze, newCustomers, monthly, feedbackStats };
      });
